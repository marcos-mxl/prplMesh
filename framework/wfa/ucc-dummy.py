...
parser.add_argument("--ports", nargs='+', default=[7000, 7001, 7002, 7003, 7004, 7005, 7006, 7007, 7008, 7009, 7010, 7011, 7012, 8888])
...
respDict = {
    ...
    'dev_get_parameter' : [b'Status,RUNNING', 'Status,COMPLETE,aLid,{}'.format(alid).encode()],
}

print("Starting threads on host {}, ports {}".format(host, ports))
threads = list()

for port in args.ports:
    print("Creating thread {}@{}".format(host, port))
    t = threading.Thread(target=handle, args=(host, port, respDict,))
    threads.append(t)
    print("Starting thread {}@{}".format(host, port))
    t.start()

for t in threads:
    print("Before joining thread {}@{}".format(host, port))
    t.join()
    print("After joining thread {}@{}".format(host, port))
